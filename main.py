...
def validate_file(file: UploadFile) -> Dict[str, Any]:
    ...
    if file.size and file.size > MAX_FILE_SIZE:
        errors.append(f"File size {file.size} exceeds maximum allowed size of {MAX_FILE_SIZE} bytes")
...
async def check_file_status(supabase: AsyncClient, file_name: str) -> Dict[str, Any]:
    ...
    try:
        db_response = await supabase.table("annotated_memes").select("image_id, file_name, annotation_status").eq("file_name", file_name).execute()
        
        if not db_response.data:
            return {"exists_in_db": False, "exists_in_storage": False, "can_upload": True}
...
async def create_or_update_db_record(supabase: AsyncClient, file_name: str, image_id: str | None = None) -> str:
    ...
    try:
        ...
        update_response = await supabase.table("annotated_memes").update({
...
async def update_status_success(supabase: AsyncClient, image_id: str, file_name: str):
    ...
    try:
        uploaded_url = f"{os.environ['SUPABASE_URL']}/storage/v1/object/public/memes/{image_id}"
...
async def update_status_failed(supabase: AsyncClient, image_id: str, file_name: str, error_msg: str):
    """Update database record on failed upload."""
    try:
        await supabase.table("annotated_memes").update({
            "annotation_status": "upload_failed",
            "err_msg": error_msg
        }).eq("image_id", image_id).execute()
    except Exception as e:
...
async def process_single_file(supabase: AsyncClient, file: UploadFile) -> Dict[str, Any]:
    ...
    try:
        ...
        image_id = await create_or_update_db_record(supabase, file_name, image_id)
        ...
        if image_id:
            try:
                ...
                logger.error(f"Failed to update error status for '{file_name}': {update_error}")
...
async def process_files_in_batches(supabase: AsyncClient, files: List[UploadFile]) -> List[Dict[str, Any]]:
...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        logger.info(f"Bulk upload completed: {len(successful_uploads)} successful, {len(failed_uploads)} failed, {len(skipped_uploads)} skipped")
        ...
        return {
            ...
            "summary": {
                ...
                "successful_files": [{"filename": r["filename"], "action": r.get("action", "unknown")} for r in successful_uploads],
                "failed_files": [{"filename": r["filename"], "error": r["error"]} for r in failed_uploads],
                "skipped_files": [{"filename": r["filename"], "message": r.get("message", "Already exists")} for r in skipped_uploads]
            }
        }
